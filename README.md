...
To start using the netdatalocal data source plugin on your Grafana environment, local or Cloud. Here are some tips to get through this depending on your setup:
...
netdatalocal is available in the Grafana Plugin catalog that can be accessed from the Grafana UI. 
...
To start the container with the netdatalocal datasource plugin already installed you just need to:
```
...
1. Retrieve the latest available release of the netdatalocal datasource plugin
1. Install the netdatalocal datasource plugin in /var/lib/grafana/plugins
1. Start grafana

#### Manual step-by-step

1. Setup your grafana docker container with the the permissions to load netdatalocal plugin
...
3. Place the contents of the `/dist` folder under the netdatalocal folder in Grafana plugins directory.
